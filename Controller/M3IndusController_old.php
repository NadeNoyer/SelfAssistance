		...
		if($this->post) {
			...
			if (!empty($this->post['codeArt']) ) {
				...
				if(isset($this->post['lecture'])) {	
					...
					$articleModel = new M3IndusModel();
					$article = $articleModel->litUMbase($codart);

					if ($article) {  // false si pas trouvé 
						...
						if (isset($this->post['update']) ){
							...
							if (!empty($this->post['unite'])) {
								...
								if (!empty($this->post['coeff'])){
									$updateArticleModel = new M3IndusModel();
									echo "toto";

									$updateArticle = $updateArticleModel->updateUNM($codart);
								} else {
				...
				} elseif (isset($this->post['lecture']))  	{
